...
void parse_command(device_t *dev, char *cmdFilePath, char *expsRegion);
int process_command(device_t *dev, char *cmd);
int change_mem(device_t *dev, char *cmd);
void useExpsRegion(device_t *dev, char *expsRegion);
...
static void show_usage() {
  printf("\nUsage: pci_debug -s <device>\n"
         ...
         "  -r <expression> execute the input expresion\n\n");
}
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "b:hs:f:r:qv:V")) != -1) {
    switch (opt) {
    case 'b':
  ...
  status = sscanf(slot, "%2x:%2x.%1x", &dev->bus, &dev->slot, &dev->function);
  if (status != 3) {
  ...
  snprintf(dev->filename, 99,
           "/sys/bus/pci/devices/%04x:%02x:%02x.%1x/resource%d", dev->domain,
           dev->bus, dev->slot, dev->function, dev->bar);
  ...
  /* Map */
  dev->maddr =
      (unsigned char *)mmap(NULL, (size_t)(dev->size), PROT_READ | PROT_WRITE,
                            MAP_SHARED, dev->fd, 0);
  ...
  /* Process commands */
  parse_command(dev, cmdFilePath, expsRegion);
  ...
  while (line != NULL) {
    ...
    if (len > 0) {
      if (line_num == 1) {
        ...
        if (dev->bar != bar) {
          printf("Warning: BAR is no compliant with the command file "
                 "(Expected: %d - Found: %d)\n",
                 dev->bar, bar);
          break;
        }
